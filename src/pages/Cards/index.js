...
import {extractLastPartAfterSlash} from '../../api/serv';
import {
  Container,
  Form,
  Input,
  ...
  handleAddCharacter = async () => {
    try {
      ...
      const raw_response = await api.get(
        `api/character/?name=${newCharacter}&count=3`,
      );
      ...
      alert(
        'Algo deu errado, verifique se colocou o nome correto do personagem!',
      );
        ...
        <List
          ...
          renderItem={({item}) => (
            <Character key={item.id}>
              ...
              <ProfileButton
                onPress={() => {
                  this.props.navigation.navigate('Personagem', {character: item});
                }}>
